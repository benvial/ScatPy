...
class Table(dict):
    ...
    def plot(self, y_fields=None, normalize=False, smooth=False, x_field=None,
        ...
        for f in y_fields:
            ...
            if smooth is False:
                plt.plot(self[x_field], y / maxy,
                         label=l_text, lw=lw, **kwargs)
            else:
                if smooth is True:
                    smooth = 0.03

                xnew = np.linspace(self[x_field].min(),
                                   self[x_field].max(), 300)
...
#            table=np.vstack([table, v])
    ...
    def smooth(self, width=None, pad_sample=5):
        ...
        gauss = np.sqrt(4 * np.log(2) / np.pi) / float(width_steps) * \
            np.exp(-4 * np.log(2) / float(width_steps)**2 * x**2)
        gauss /= gauss.sum()
...
class AVGTable(ResultTable):
    ...
    def __init__(self, fname=None, folder=None, **kwargs):
        ...
        if hdr == 31:
            self.summary = AVGSummaryTable(
                fname, folder, npol=1, ncomps=ncomps, **kwargs)
        else:
            self.summary = AVGSummaryTable(
                fname, folder, npol=2, ncomps=ncomps, **kwargs)
...
class QTable(ResultTable):
    ...
    def __init__(self, fname=None, folder=None, **kwargs):
        ...
        ResultTable.__init__(self, fname, hdr_lines, [
                             11, 11, 11, 11, 11, 12, 11, 11, 6], folder=folder, **kwargs)
        self.x_field = 'wave'
        self.y_fields = ['Q_ext']
...
class QTable2(ResultTable):
    ...
    def __init__(self, fname=None, folder=None, **kwargs):
        ...
        hdr_lines = _find_header_len(fname, folder, 'aeff')
        ResultTable.__init__(self, fname, hdr_lines, [
                             10, 11, 12, 12, 12], folder=folder, **kwargs)
        self.x_field = 'wave'
        self.y_fields = ['Q_pha']
...
class MTable(ResultTable):
    ...
    def __init__(self, fname=None, **kwargs):
        ...
        ResultTable.__init__(
            self, fname, 7, [10, 11, 10, 10, 10, 10], **kwargs)
    ...
    def refresh(self):
        ...
        with open(fname, 'Ur') as f:
            ...
            for l in self.col_lbl[1:]:
                ...
                self.interps[cl] = interp1d(
                    self['wave'][::step], self[l][::step])
...
class ShapeTable(dict):
    ...
    def show(self, show_now=True, *args, **kwargs):
        ...
        if 'mask_points' in kwargs:
            ...
            print(
                'Plotting only a subset. Specify mask_points=None or an integer to force skipping value')
...
class TargetTable(ShapeTable):
    ...
    def _load(self, f):
        ...
        l = f.readline()
        dat = np.asarray(
            list(map(int, split_string(l, c_width))), dtype=np.int)
        l = f.readline()
        while l != '':
            dat = np.vstack(
...
class EnTable(dict):
    ...
    def show(self, field='Etot2', phase=0.0, mask_points=None, show_now=True):
        ...
        if show_now:
            ...
            warnings.warn(
                'Use mayavi.mlab.show() to display the figure when you are ready')
...
class ResultCollection(OrderedDict):
    ...
    def plot(self, fields=None, label=None, normalize=False, lw=2, **kwargs):
        ...
        for (key, table) in self.items():
            ...
            table.plot(fields, normalize=normalize,
                       label=l_txt, lw=lw, **kwargs)
...
class FileCollection(ResultCollection):
    ...
    def make_table(self):
        ...
        cols = list(self[list(self.keys())[0]].keys())
        table = Table()
        for c in cols:
...
#            raise (IOError, 'Archive %s not found'% os.path.join(folder, zname))
 #       else:
        with zipfile.ZipFile(os.path.join(folder, zname), 'rb') as z:
...
class SCAHyperSpace():
    ...
    def refresh(self):
        ...
        W = dict(list(zip(self.w_range, list(range(len(self.w_range))))))
        R = dict(list(zip(self.r_range, list(range(len(self.r_range))))))
        Beta = dict(
            list(zip(self.beta_range, list(range(len(self.beta_range))))))
        Theta = dict(
            list(zip(self.theta_range, list(range(len(self.theta_range))))))
        Phi = dict(list(zip(self.phi_range, list(range(len(self.phi_range))))))

        if self.zfile:
            with zipfile.ZipFile(self.zfile, 'rb') as z:
                flist = z.namelist()
        ...
        for f in flist:
            ...
            if len(self.Epol) == 1:
                dat = np.array((sca['Q_ext'], sca['Q_abs'],
                                sca['Q_sca'])).transpose()
            elif len(self.Epol) == 2:
                dat = np.array((sca['Q_ext'], sca['Q_abs'],
                                sca['Q_sca'], sca['Q_pol'])).transpose()
    ...
    def dichroism(self):
        if len(self.Epol_range) < 2:
            raise ValueError(
                'Cannot calculate dichroism with only one polarization')
...
def dichroism_from_avg(fields, folder=None):
    ...
    for files in glob.glob("w???r???.avg"):
        ...
        t = [s.wave, s.J2_Qext - s.J1_Qext, s.J2_Qabs -
             s.J1_Qabs, s.J2_Qsca - s.J1_Qsca]
        table.append(t)
